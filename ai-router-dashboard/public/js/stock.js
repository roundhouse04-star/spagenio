...
async function loadOrderBook() {
  ...
  try {
    ...
    const trade = tradeRes.ok ? (await tradeRes.json()).trade || {} : {};
    const quote = quoteRes.ok ? (await quoteRes.json()).quote || {} : {};
    const bar   = barRes.ok  ? (await barRes.json()).bar   || {} : {};
    const latestPrice = parseFloat(trade.p) || parseFloat(quote.ap) || 0;
    const askPrice = parseFloat(quote.ap) || 0;
    const bidPrice = parseFloat(quote.bp) || 0;
    const askSize  = parseInt(quote.as)   || 0;
    ...
    const low  = parseFloat(bar.l) || 0;
    const volume = bar.v || 0;
    const change    = open > 0 ? latestPrice - open : 0;
    const changePct = open > 0 ? (change / open * 100) : 0;
    const isUp = change >= 0;
    const upColor = '#16a34a'; const dnColor = '#dc2626';
    const priceColor = isUp ? upColor : dnColor;
    if (priceEl) {
      priceEl.innerHTML = `
        <div style="margin-bottom:12px;">
          <div style="font-size:1.6rem;font-weight:800;color:${priceColor};">$${latestPrice.toFixed(2)}</div>
          <div style="font-size:0.82rem;font-weight:700;color:${priceColor};margin-top:2px;">
            ${isUp?'▲':'▼'} ${Math.abs(change).toFixed(2)} (${Math.abs(changePct).toFixed(2)}%)
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:0.78rem;">
          <div style="background:#161B22;border-radius:6px;padding:6px 8px;">
            <div style="color:#6B7280;">시가</div><div style="font-weight:700;">$${open.toFixed(2)}</div>
          </div>
          <div style="background:#161B22;border-radius:6px;padding:6px 8px;">
            <div style="color:#6B7280;">전일종가</div><div style="font-weight:700;">$${latestPrice.toFixed(2)}</div>
          </div>
          <div style="background:#161B22;border-radius:6px;padding:6px 8px;">
            <div style="color:#16a34a;">고가</div><div style="font-weight:700;color:#16a34a;">$${high.toFixed(2)}</div>
          </div>
          <div style="background:#161B22;border-radius:6px;padding:6px 8px;">
            <div style="color:#dc2626;">저가</div><div style="font-weight:700;color:#dc2626;">$${low.toFixed(2)}</div>
          </div>
        </div>
        <div style="margin-top:10px;font-size:0.75rem;color:#6B7280;border-top:1px solid #2A2A2A;padding-top:8px;">
          거래량: ${volume ? Number(volume).toLocaleString() : '-'}
        </div>`;
    }
    if (tableEl) {
      ...
      const bidBarW = Math.round(bidSize / maxSize * 100);
      tableEl.innerHTML = `
        <div style="font-size:0.75rem;color:#9CA3AF;margin-bottom:6px;font-weight:700;">매도 호가</div>
        <div style="background:#2D1515;border-radius:8px;padding:10px 12px;margin-bottom:4px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            <span style="font-size:0.72rem;color:#6B7280;">잔량</span>
            <span style="font-size:0.72rem;color:#6B7280;">매도호가</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:0.82rem;color:#9CA3AF;">${askSize.toLocaleString()}</span>
            <span style="font-size:1.15rem;font-weight:800;color:#dc2626;">$${askPrice > 0 ? askPrice.toFixed(2) : '-'}</span>
          </div>
          <div style="margin-top:6px;height:6px;background:#3D1F1F;border-radius:3px;overflow:hidden;">
            <div style="height:100%;width:${askBarW}%;background:#dc2626;border-radius:3px;float:right;"></div>
          </div>
        </div>
        <div style="text-align:center;font-size:0.72rem;color:#6B7280;padding:6px 0;border-top:1px solid #2A2A2A;border-bottom:1px solid #2A2A2A;margin:4px 0;">
          스프레드 ${spread > 0 ? '$'+spread.toFixed(2)+' ('+spreadPct.toFixed(3)+'%)' : '-'}
        </div>
        <div style="font-size:0.75rem;color:#9CA3AF;margin-bottom:6px;margin-top:4px;font-weight:700;">매수 호가</div>
        <div style="background:#152D1F;border-radius:8px;padding:10px 12px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            <span style="font-size:0.72rem;color:#6B7280;">잔량</span>
            <span style="font-size:0.72rem;color:#6B7280;">매수호가</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:0.82rem;color:#9CA3AF;">${bidSize.toLocaleString()}</span>
            <span style="font-size:1.15rem;font-weight:800;color:#16a34a;">$${bidPrice > 0 ? bidPrice.toFixed(2) : '-'}</span>
          </div>
          <div style="margin-top:6px;height:6px;background:#1F3D2A;border-radius:3px;overflow:hidden;">
            <div style="height:100%;width:${bidBarW}%;background:#16a34a;border-radius:3px;"></div>
          </div>
        </div>
        <div style="margin-top:10px;font-size:0.7rem;color:#4B5563;text-align:center;">Alpaca 최우선 호가</div>`;
    }
    if (statusEl) {
      statusEl.textContent = new Date().toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit',second:'2-digit'})+' 기준';
    }
    clearTimeout(_obRefreshTimer);
    _obRefreshTimer = setTimeout(() => loadOrderBook(), 30000);
  } catch(e) {
...
const _origSelectStockOB = window.selectStock;
if (typeof _origSelectStockOB === 'function') {
  window.selectStock = function(sym, name) {
    _origSelectStockOB(sym, name);
    setTimeout(() => { if (document.getElementById('tradeSymbol')?.value === sym) loadOrderBook(); }, 150);
  };
}
