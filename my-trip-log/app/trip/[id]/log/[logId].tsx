...
import { getTripLog, updateTripLog, deleteTripLog } from '@/db/logs';
...
export default function EditLogScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const log = await getTripLog(logIdNum);
        if (!log) {
  ...
  }, [canSave, saving, logIdNum, logDate, title, content, images, location, weather, mood]);

  const handleDelete = useCallback(() => {
    Alert.alert('기록 삭제', '이 기록을 삭제하시겠어요? 되돌릴 수 없습니다.', [
...
function createStyles(c: ColorPalette) {
  ...
  datePlaceholder: { color: c.textTertiary },
