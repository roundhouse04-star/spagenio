  ...
  try {
    ...
    try {
      ...
      for (const e of data.expenses || []) {
        await db.runAsync(
          `INSERT INTO expenses (id, trip_id, expense_date, category, title, amount, currency, amount_in_home_currency, exchange_rate, payment_method, memo, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [e.id, e.trip_id, e.expense_date, e.category, e.title, e.amount, e.currency, e.amount_in_home_currency, e.exchange_rate, e.payment_method, e.memo, e.created_at]
        );
      }
