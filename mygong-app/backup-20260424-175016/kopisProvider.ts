...
// 출연진 정밀 매칭
const ENABLE_CAST_MATCHING = true;
...
export async function searchKopisEvents(
  ...
  console.log('[kopis] STEP 1/3: Name search starting...');
  ...
  let all = await runThrottled(tasks, MAX_CONCURRENCY);
  let rawCount = all.flat().length;
  let seen = new Set<string>();
  let dedup = all.flat().filter(e => {
    if (seen.has(e.mt20id)) return false;
    seen.add(e.mt20id);
    return true;
  });

  console.log('[kopis] STEP 1/3: ' + rawCount + ' raw -> ' + dedup.length + ' dedup (name search)');

  if (mode === 'full' && dedup.length < 30) {
    console.log('[kopis] STEP 2/3: Genre search starting (recent 2 years only)...');
    const genres = getGenresForTag(artistTag);
    const recentStart = new Date(today);
    recentStart.setFullYear(recentStart.getFullYear() - 2);
    const recentChunks = splitByMonth(recentStart, end);
    
    console.log('[kopis] STEP 2/3: genres=[' + genres.join(',') + '] chunks=' + recentChunks.length);
    
    const genreTasks: Array<() => Promise<KopisRawEvent[]>> = [];
    for (const genre of genres) {
      for (const [stdate, eddate] of recentChunks) {
        genreTasks.push(() => fetchList({
          stdate, eddate, shcate: genre,
        }));
      }
    }
    
    const genreResults = await runThrottled(genreTasks, MAX_CONCURRENCY);
    const genreRawCount = genreResults.flat().length;
    
    for (const event of genreResults.flat()) {
      if (!seen.has(event.mt20id)) {
        seen.add(event.mt20id);
        dedup.push(event);
      }
    }
    
    console.log('[kopis] STEP 2/3: ' + genreRawCount + ' raw -> ' + dedup.length + ' total after merge');
  } else if (mode === 'full') {
    console.log('[kopis] STEP 2/3: Skipped (sufficient results: ' + dedup.length + ')');
  }

  if (mode === 'full' && dedup.length > 17) {
    console.log('[kopis] STEP 3/3: Cast verification starting...');
    const nameSearchCount = Math.min(17, dedup.length);
    const toVerify = dedup.slice(nameSearchCount);
    const verified = await verifyCastMatchingFast(toVerify, uniqueNames);
    const finalResults = [...dedup.slice(0, nameSearchCount), ...verified];
    
    console.log('[kopis] STEP 3/3: ' + toVerify.length + ' -> ' + verified.length + ' verified (cast matching)');
    console.log('[kopis] FINAL: ' + finalResults.length + ' total');
    return finalResults;
  }

  console.log('[kopis] FINAL: ' + dedup.length + ' total (no filtering needed)');
  return dedup;
}
...
async function verifyCastMatchingFast(
  ...
  const tasks = events.map(ev => async () => {
    ...
    for (const name of names) {
      if (includesName(detail.prfcast, name)) {
        return ev;
      }
    }
