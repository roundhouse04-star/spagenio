...
# ── 경로 계산 ────────────────────────────────────────────
APP_DIR="$(cd "$(dirname "$0")" && pwd)"   # mygong-app
PROJECT="$(dirname "$APP_DIR")"            # spagenio
APP_NAME="$(basename "$APP_DIR")"          # mygong-app

# ── 커밋 메시지 ──────────────────────────────────────────
MSG="${1:-"update: $APP_NAME"}"

echo -e "${BLUE}==========================================${NC}"
echo -e "${BLUE}  내공연관리 (mygong-app) 배포${NC}"
echo -e "${BLUE}==========================================${NC}"
...
# ── 중첩 폴더 자동 감지 및 제거 ──────────────────────────
if [ -d "$APP_DIR/mygong-app" ]; then
  echo -e "${YELLOW}⚠️  mygong-app 중첩 폴더 감지됨! 자동 제거합니다...${NC}"
  rm -rf "$APP_DIR/mygong-app"
...
CHANGES=$(git status --porcelain mygong-app/ | wc -l | tr -d ' ')
...
# ── 민감 파일 경고 ───────────────────────────────────────
SENSITIVE=$(git status --porcelain mygong-app/ | grep -E "\.(env|key|pem|p12)$|secret|password" || true)
if [ -n "$SENSITIVE" ]; then
