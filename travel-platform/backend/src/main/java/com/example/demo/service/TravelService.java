...
public class TravelService {
    ...
    public Booking createBooking(Booking booking) {
        if (booking.getPackageId() == null) {
            throw new IllegalArgumentException("package_id_required");
        }
        if (booking.getCustomerName() == null || booking.getCustomerName().isBlank()) {
            throw new IllegalArgumentException("customer_name_required");
        }
        ...
        if (booking.getTravelers() <= 0) {
            throw new IllegalArgumentException("travelers_required");
        }
