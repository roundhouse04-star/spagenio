      ...
      <div style={{ padding: '8px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: '#8A919C' }}>{ad.target_country || 'ALL'} · {ad.target_city || 'ALL'}</div>
          </div>
        </div>
        <span style={{ fontSize: 10, color: '#B8BCC4', background: '#FAFAF8', padding: '2px 8px', borderRadius: 8 }}>Ad</span>
      </div>
...
// Autoplay video (on scroll play/pause, mute toggle)
...
function PromoCard({ promo }) {
  const typeColor = promo.type === 'ad'
   ? { bg: '#fffbeb', border: '#fde68a', badge: '#d97706', badgeText: 'Ad' }
    : promo.type === 'event'
     ? { bg: '#f0fdf4', border: '#bbf7d0', badge: '#16a34a', badgeText: 'Event' }
      : { bg: '#EEEDEA', border: '#c7d2fe', badge: '#1E2A3A', badgeText: 'Notice' };
...
function SNSPostCard({ post, currentUserId, onOpen, onProfile, onLike, onReport }) {
  ...
  return (
    <div style={{ background: 'white', borderBottom: '1px solid #F0EEE9' }}>
      ...
      {/* LIKE can */}
      {likeCount > 0 && (
        <div style={{ padding: '0 14px 4px', fontSize: 13, fontWeight: 700, color: '#1E2A3A' }}>LIKE {likeCount}</div>
      )}
          ...
          <div style={{ marginTop: 3 }}>
            <span style={{ fontSize: 13, color: '#4A5568', lineHeight: 1.5 }}>
              ...
              {post.content.length > 80 &&!showMore && (
                <span style={{ color: '#8A919C', cursor: 'pointer' }} onClick={() => setShowMore(true)}>... View more</span>
              )}
            </span>
          </div>
        )}
      ...
      {commentCount > 0 && (
        <button onClick={() => onOpen?.(post)}
          ...
          COMMENTS {commentCount} View all
        </button>
      )}

      {/* Hashtags + TRAVELStyle */}
...
export default function Feed({ currentUser, onOpenPost, onProfile, onTagClick }) {
  ...
  const sortByPreference = (posts, preferredStyles) => {
    // User Nationality & Wish list country Fetch
    ...
    // Country code → country list map
    const COUNTRY_MAP = {
      JP: ['Japan', 'japan'], US: ['USA', 'Hawaii', 'usa'], FR: ['France', 'france'],
      IT: ['Italy', 'italy', 'Rome'], TH: ['Thailand', 'Bangkok', 'thailand'],
      ID: ['Bali', 'Indonesia', 'bali'], ES: ['Spain', 'spain', 'Barcelona'],
      GB: ['UK', 'London', 'london'], AU: ['Australia', 'australia'],
      SG: ['Singapore', 'singapore'], VN: ['Vietnam', 'vietnam'],
      CN: ['China', 'china'], HK: ['Hong Kong', 'hong kong'],
      TR: ['Turkey', 'Istanbul'], MA: ['Morocco', 'Marrakesh'],
      MX: ['Mexico', 'mexico'], CZ: ['Czechia', 'Prague'],
      NL: ['Netherlands', 'Amsterdam'], AE: ['Dubai', 'dubai'],
      KR: ['Korea', 'Seoul', 'korea'],
    };
    ...
    return [...posts].sort((a, b) => {
      // 1st priority: Wish list country
  ...
  const load = async () => {
    ...
    try {
      if (tab === 'following' && currentUser?.id) {
        ...
        // composite scoring: LIKEx3 + COMMENTSx2 + Time value
    ...
    <div style={{ width: 680, margin: '0 auto', Width: '100%' }}>
      ...
      {loading? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', color: '#8A919C' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: '#4A5568', marginBottom: 6 }}>posts none</div>
          <div style={{ fontSize: 13 }}>First Travel stories post!</div>
        </div>
      ) : (
        <>
          ...
          <div ref={sentinelRef} style={{ height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '10px 0' }}>
            {loadingMore && <div style={{ fontSize: 12, color: '#8A919C' }}>Loading...</div>}
            {!hasMore && posts.length > 0 && <div style={{ fontSize: 12, color: '#B8BCC4' }}>You've seen all posts ✓</div>}
          </div>
        </>
      )}
