  ...
  return (
    <form className="search-panel" onSubmit={submit}>
      <div className="search-grid">
        <label>목적지<input value={form.destination} onChange={(e) => update('destination', e.target.value)} placeholder="예: Osaka, Tokyo, Bali" /></label>
        <label>출발일<input type="date" value={form.startDate} onChange={(e) => update('startDate', e.target.value)} /></label>
        <label>도착일<input type="date" value={form.endDate} onChange={(e) => update('endDate', e.target.value)} /></label>
        <label>인원수<input type="number" min="1" max="10" value={form.travelers} onChange={(e) => update('travelers', Number(e.target.value))} /></label>
        <label>예산<select value={form.budget} onChange={(e) => update('budget', e.target.value)}>{budgetOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></label>
        <label>여행타입<select value={form.category} onChange={(e) => update('category', e.target.value)}>{categoryOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></label>
      </div>
    ...
    <div className="page-stack">
      <section className="hero-card">
        <div className="hero-copy">
          ...
          <p>지금 버전은 운영에 바로 붙일 수 있도록 여행 검색, 추천 상품, 예약 생성, 최근 예약 확인 흐름까지 모두 연결되어 있습니다.</p>
            ...
            <div className="booking-row clickable" key={booking.id} onClick={() => onOpenBookingDetail(booking)}>
              ...
              <div style={{ textAlign: 'right' }}>
                <span className={`status-badge ${booking.status === 'CANCELLED' ? 'cancelled' : 'confirmed'}`}>
                  {booking.status === 'CANCELLED' ? '취소됨' : '예약확정'}
                </span>
